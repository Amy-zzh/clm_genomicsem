...
#write in the model
model <- "BF=~NA*AF + HF + Stroke + PAD + CAD + VHD + NAFLD + ALT + AST + GGT + ALP + BMI + WC + BFP + T2D + HbA1c + HTN + nonHDL + HDL + logTG + CRP
...
results<-userGWAS(covstruc=LDSCoutput, SNPs=sumstats[chunk,], model=model, parallel=T, sub=sub, cores=12, GC="none",smooth_check=TRUE, printwarn=TRUE,toler=1e-50,fix_measurement=TRUE,Q_SNP=TRUE)
